...
from rules.update_runner_messages import (
    ...
    LABEL_MAPPING_SHEET,
    LABEL_SHEET_MAPPING,
    LABEL_UPDATE_PROGRAM,
    LABEL_UPDATE_PROGRAM_TEMPLATE,
...
def selected_mapping_sheet_path() -> Path:
    ...
    if len(mapping_paths) > 1:
        names = ", ".join(path.name for path in mapping_paths)
        raise ValueError(
            MULTIPLE_MAPPING_TEMPLATE.format(
...
def selected_program_paths() -> list[Path]:
    paths = update_program_paths()
    if paths:
        return paths

    if DEFAULT_UPDATE_SOURCE_PATH.exists() and DEFAULT_UPDATE_SOURCE_PATH.is_file():
        return [DEFAULT_UPDATE_SOURCE_PATH]

    return []


def validate_inputs(output_dir: Path) -> None:
    validate_folder_exists(DEFAULT_MAPPING_SHEET_DIR, LABEL_MAPPING_SHEET)
    validate_folder_exists(DEFAULT_DCLGEN_DIR, LABEL_DCLGEN_FOLDER)
    validate_folder_exists(DEFAULT_UPDATE_PROGRAM_DIR, LABEL_UPDATE_PROGRAM)
    ...
    for index, dclgen_path in enumerate(dclgen_files, start=1):
        validate_file_exists(dclgen_path, LABEL_DCLGEN_TEMPLATE.format(index=index))

    for index, copybook_path in enumerate(selected_copybook_paths(), start=1):
        validate_file_exists(
            copybook_path, LABEL_COPYBOOK_TEMPLATE.format(index=index)
        )

    program_files = selected_program_paths()
    if not program_files:
        raise FileNotFoundError(
            NO_PROGRAM_FILE_TEMPLATE.format(folder=DEFAULT_UPDATE_PROGRAM_DIR)
        )

    for index, program_path in enumerate(program_files, start=1):
        validate_file_exists(
            program_path, LABEL_UPDATE_PROGRAM_TEMPLATE.format(index=index)
        )
